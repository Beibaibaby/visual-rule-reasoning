...
logfile="./expri/"+args.dataset+"/expri_oct_30_number="+str(args.num_label_raven)+"_gpu="+args.gpu+"_drule_ws.txt"
...
def train(epoch):
    ...
    for batch_idx in trange(len(train_loader_iter), ncols=ncols_const):
        ...
        for i in range(args.k_aug):
            ...
            image_unlabel_temp_strong = Variable(data_auger_strong(image_unlabel), requires_grad=True).to(device)
            stu_model_output = model(image_unlabel_temp)
            stu_model_output_strong= model(image_unlabel_temp_strong)
            ...
            list_stu_model_output_strong.append(stu_model_output_strong)
            '''
